            ...
            if (cluster.isMaster && !process.env.NOPARALLEL) {
                ...
                if(mappingSource){
                    mappingSource.slice(process.env.from || 0, process.env.to || mappingSource.length).forEach(function(element){
                        ...
                        localizationSearchItems = element[originalMappingElementKey].split(",");

                        const localizationsMatch = subcellLocAgesProteinsSource
                            .filter(function(subCellLoc){
                                return localizationSearchItems.find(function(possibleMatche){
                                    return possibleMatche == subCellLoc.uniprotac;
                                });
                            })
                            .map(function(subCellLoc){
                                return subCellLoc.consensus_sl.split(". ");
                            })
                        ...
                        protein.localizations = {
                            localizations: localizationsMatch,
